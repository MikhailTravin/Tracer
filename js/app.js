    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) {
                ...
                menuShadow.addEventListener("click", (function(e) {
                    menuClose();
                }));
            }
        }
        ...
        let formValidate = {
            ...
            addError(formRequiredItem) {
                ...
                if (formRequiredItem.dataset.error) formRequiredItem.parentElement.insertAdjacentHTML("beforeend", `<div class="form__error">\t<img src="../img/icons/form-error.svg" alt="">${formRequiredItem.dataset.error}</div>`);
            },
        ...
        if (document.querySelector(".process__slider")) new core(".process__slider", {
            modules: [ Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
        ...
        function mouseParallax() {
            const page = document.querySelector(".main-home__image");
            ...
            function setParallaxAnimationStyle(e) {
                ...
                parallaxItems.forEach((parallaxItem => {
                    const valueX = parallaxItem.dataset.prxValue ? +parallaxItem.dataset.prxValue : 3;
                    const valueY = parallaxItem.dataset.prxValue ? +parallaxItem.dataset.prxValue : 3;
                    parallaxItem.style.cssText = `\n\t\t\t\ttransform: translate(${posX / valueX}%,${posY / valueY}%);\n\t\t\t`;
                }));
        ...
        function writeTextByJS(id, text, speed) {
            ...
            ele.classList.add("carriage");
            var interval = setInterval((function() {
                if (!txt[0]) {
                    ele.classList.remove("carriage");
                    return clearInterval(interval);
                }
                ele.innerHTML += txt.shift();
            }), void 0 != speed ? speed : 80);
            return false;
        }
        writeTextByJS(".main-home__subtitle", [ "Эффектные сайты \n", "с эффективным решением \n" ]);
        let mainQuantityNum = document.querySelectorAll(".number__num");
        let limit = 0;
        window.addEventListener("scroll", (function() {
            if (mainQuantityNum) {
                if (limit == mainQuantityNum.length) return;
                for (let i = 0; i < mainQuantityNum.length; i++) {
                    let pos = mainQuantityNum[i].getBoundingClientRect().top;
                    let win = window.innerHeight - 40;
                    if (pos < win && "0" === mainQuantityNum[i].dataset.stop) {
                        mainQuantityNum[i].dataset.stop = 1;
                        let x = 0;
                        limit++;
                        let int = setInterval((function() {
                            x += Math.ceil(mainQuantityNum[i].dataset.to / 50);
                            mainQuantityNum[i].innerText = x;
                            if (x > mainQuantityNum[i].dataset.to) {
                                mainQuantityNum[i].innerText = mainQuantityNum[i].dataset.to;
                                clearInterval(int);
                            }
                        }), 60);
                    }
